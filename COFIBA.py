...
class COFIBAAlgorithm(LinUCBAlgorithm):
    ...
    def decide(self, pool_articles, userID):
        ...
        for x in pool_articles:
            ...
            self.users[userID].updateParametersofClusters(self.Uclusters[itemClusterNum], userID, self.UGraph,
                                                          self.users)
    ...
    def updateUserClusters(self, userID, articlePicked_FeatureVector, itemClusterNum):
        ...
        for j in range(n):
            diff = math.fabs(
                np.dot(self.users[userID].UserTheta, articlePicked_FeatureVector) - np.dot(self.users[j].UserTheta,
                                                                                           articlePicked_FeatureVector))
            CB = self.alpha_2 * (np.sqrt(np.dot(np.dot(articlePicked_FeatureVector, self.users[userID].AInv),
                                                articlePicked_FeatureVector)) + np.sqrt(
                np.dot(np.dot(articlePicked_FeatureVector, self.users[j].AInv),
    ...
    def updateItemClusters(self, userID, chosenItem, itemClusterNum, articlePool):
        ...
        for a in articlePool:
            if self.IGraph[chosenItem.id][a.id] == 1:
                ...
                for i in range(n):
                    diff = math.fabs(
                        np.dot(self.users[userID].UserTheta, a.featureVector) - np.dot(self.users[i].UserTheta,
                                                                                       a.featureVector))
                    CB = self.alpha_2 * (
                    np.sqrt(np.dot(np.dot(a.featureVector, self.users[userID].AInv), a.featureVector)) + np.sqrt(
                        np.dot(np.dot(a.featureVector, self.users[i].AInv), a.featureVector))) * np.sqrt(
